  ...
  */
  public function body( $params ) {
    ...
    echo $this->title(array(
                        'title' => __('Blocked IPs', WDFM()->prefix),
    ...
    <table class="adminlist table table-striped wp-list-table widefat fixed pages">
      <thead>
        <tr>
          ...
          <?php echo WDW_FM_Library::ordering('ip', $orderby, $order, __('IP', WDFM()->prefix), $page_url, 'column-primary col_type wd-left'); ?>
        </tr>
        <tr id="tr">
          <th></th>
          ...
          <td>
            ...
            <input type = button class="button action" id="add_ip" onclick="if (fm_check_required('fm_ip', '<?php _e('IP', WDFM()->prefix); ?>')) {return false;}  fm_insert_blocked_ip('blocked_ips'); " value="<?php _e('Add IP', WDFM()->prefix); ?>">
            <div class="loading"><img src="<?php echo WDFM()->plugin_url ?>/images/loading.gif"></div>
          </td>
        </tr>
        ...
        if ( $rows_data ) {
          foreach ( $rows_data as $row_data ) {
            ...
            <tr id="tr_<?php echo $row_data->id; ?>" <?php echo $alternate; ?>>
              ...
              <td class="column-primary" id="td_ip_<?php echo $row_data->id; ?>" data-colname="<?php _e('IP', WDFM()->prefix); ?>">
                ...
                <div class="row-actions">
                  ...
                  <span id="td_edit_<?php echo $row_data->id; ?>">
                    <a class="pointer" onclick="fm_edit_ip(<?php echo $row_data->id; ?>);"> <?php _e('Edit', WDFM()->prefix); ?> </a>
                    |
                  </span>

                  <span class="trash" id="td_delete_<?php echo $row_data->id; ?>">
                    <a class="pointer" onclick="if (!confirm('<?php echo addslashes(__('Do you want to delete selected item?', WDFM()->prefix)); ?>')) {return false;}  fm_delete_ip(<?php echo $row_data->id; ?>) "><?php _e('Delete', WDFM()->prefix); ?></a>
                  </span>
                </div>
              </td>
            </tr>
