...
class FMViewManage_fmc extends FMAdminView_fmc {
  ...
  public function display($params) {
    ...
    $form_attr = array(
      'id' => 'manage_form',
      'class' =>'wd-form',
      'action' => add_query_arg(array('page' => 'manage' . WDCFM()->menu_postfix), 'admin.php'),
    );
  ...
  public function body($params) {
    ...
    <table class="adminlist table table-striped wp-list-table widefat fixed pages">
      <thead>
        <tr>
          ...
          <?php echo WDW_FMC_Library::ordering('title', $orderby, $order, __('Title', WDCFM()->prefix), $page_url, 'col_title column-primary wd-left'); ?>
          <?php echo WDW_FMC_Library::ordering('type', $orderby, $order, __('Type', WDCFM()->prefix), $page_url, 'col_type wd-left'); ?>
          <th class="col_count wd-left"><?php _e('Submissions', WDCFM()->prefix); ?></th>
          <?php echo WDW_FMC_Library::ordering('id', $orderby, $order, __('Shortcode', WDCFM()->prefix), $page_url, 'wd-center'); ?>
          <th class="col_function wd-center"><?php _e('PHP function', WDCFM()->prefix); ?></th>
        </tr>
      </thead>
      <tbody>
        <?php
        if ( $rows_data ) {
          foreach ( $rows_data as $row_data ) {
            ...
            <tr id="tr_<?php echo $row_data->id; ?>" <?php echo $alternate; ?>>
              ...
              <td class="column-primary" data-colname="<?php _e('Title', WDCFM()->prefix); ?>">
                <strong>
                  <?php
                  if ( !$old ) {
                   ?>
                  ...
                  if ( !$row_data->published ) {
                    ?>
                    —
                    <span class="post-state"><?php _e('Unpublished', WDCFM()->prefix); ?></span>
                    <?php
                  }
                  ?>
                </strong>
                <div class="row-actions">
                  ...
                  if ( !$old ) {
                    ?>
                  <span>
                    <a href="<?php echo $edit_url; ?>"><?php _e('Edit', WDCFM()->prefix); ?></a>
                    |
                  </span>
                    <?php
                  }
                  ?>
                  <span>
                    <a href="<?php echo $duplicate_url; ?>"><?php _e('Duplicate', WDCFM()->prefix); ?></a>
                    |
                  </span>
                  <span>
                    <a href="<?php echo $publish_url; ?>"><?php echo ($row_data->published ? __('Unpublish', WDCFM()->prefix) : __('Publish', WDCFM()->prefix)); ?></a>
                    |
                  </span>
                  <span class="trash">
                    <a onclick="if (!confirm('<?php echo addslashes(__('Do you want to delete selected item?', WDCFM()->prefix)); ?>')) {return false;}" href="<?php echo $delete_url; ?>"><?php _e('Delete', WDCFM()->prefix); ?></a>
                    |
                  </span>
                  <span>
                   <a href="<?php echo $preview_url; ?>" target="_blank"><?php _e('Preview', WDCFM()->prefix); ?></a>
                  </span>
                </div>
                <button class="toggle-row" type="button">
                  <span class="screen-reader-text"><?php _e('Show more details', WDCFM()->prefix); ?></span>
                </button>
              </td>
              <td data-colname="<?php _e('Type', WDCFM()->prefix); ?>">
                <?php echo ucfirst($row_data->type); ?>
                <div class="row-actions">
                  <span>
                    <a href="<?php echo add_query_arg(array('task' => 'display_options', 'current_id' => $row_data->id), $page_url); ?>"><?php _e('Set display options', WDCFM()->prefix); ?></a>
                  </span>
                </div>
              </td>
              <td data-colname="<?php _e('Submissions', WDCFM()->prefix); ?>">
                <?php
                if ($row_data->submission_count != 0) {
                  ?>
                <a title="<?php _e('View sumbissions', WDCFM()->prefix); ?>" target="_blank" href="<?php echo add_query_arg(array(
                                                                    'page' => 'submissions' . WDCFM()->menu_postfix,
                                                                    'task' => 'display',
              ...
              <td data-colname="<?php _e('Shortcode', WDCFM()->prefix); ?>">
                <input type="text" value='[wd_contact_form  id="<?php echo $row_data->id; ?>"]' onclick="fm_select_value(this)" size="12" readonly="readonly" />
              </td>
              <td data-colname="<?php _e('PHP function', WDCFM()->prefix); ?>">
                <input type="text" value='&#60;?php wd_contact_form_maker(<?php echo $row_data->id; ?>, "<?php echo $row_data->type; ?>"); ?&#62;' onclick="fm_select_value(this)"  readonly="readonly" />
              </td>
            </tr>
            <?php
          }
          ...
          echo WDW_FMC_Library::no_items('forms');
        }
        ?>
      </tbody>
    </table>
    ...
    wp_add_inline_script('jquery-ui-datepicker', WDW_FMC_Library::localize_ui_datepicker());
      ...
      default_theme  = '<?php echo $default_theme; ?>';
			theme_edit_url = '<?php echo add_query_arg( array('page' => 'themes' . WDCFM()->menu_postfix, 'task' =>'edit'), $page_url); ?>';
			jQuery(document).ready(function () {
				set_theme();
			});
		</script>
		<form class="wrap" id="manage_form" method="post" autocomplete="off" action="admin.php?page=manage<?php echo WDCFM()->menu_postfix; ?>">
      <?php
      ...
      ?>
			...
			<div class="fm-page-header">
				<div class="wd-page-title wd-header">
					<h1 class="wp-heading-inline"><?php _e('Form Title', WDCFM()->prefix); ?></h1>
					...
					<?php
						if ( isset($row->backup_id) ) {
						  if ( $row->backup_id != "" ) {
							...
							if ( $backup_id ) { ?>
							  <button class="button redo-button button-large" onclick="if (fm_check_required('title', '<?php _e('Form Title', WDCFM()->prefix); ?>') || !FormManageSubmitButton()) {return false;}; jQuery('#saving_text').html('<?php _e('Redo', WDCFM()->prefix); ?>');fm_set_input_value('task', 'redo');">
								<?php _e('Redo', WDCFM()->prefix); ?>
							  </button>
							  <?php
							}
							$backup_id = $prev_backup_id;
							if ( $backup_id ) { ?>
							  <button class="button undo-button button-large" onclick="if (fm_check_required('title', '<?php _e('Form Title', WDCFM()->prefix); ?>') || !FormManageSubmitButton()) {return false;}; jQuery('#saving_text').html('<?php _e('Undo', WDCFM()->prefix); ?>');fm_set_input_value('task', 'undo');">
								<span></span>
								<?php _e('Undo', WDCFM()->prefix); ?>
							  </button>
							  <?php
							}
						  }
					...
					?>
					<button class="button button-primary button-large" onclick="if (fm_check_required('title', '<?php _e('Form Title', WDCFM()->prefix); ?>') || !FormManageSubmitButton()) {return false;}; fm_set_input_value('task', 'apply');">
					<?php
					  if ($row->title) {
						_e('Update', WDCFM()->prefix);
					  }
					  else {
						_e('Publish', WDCFM()->prefix);
					  }
					  ?>
					</button>
					<button class="button preview-button button-large"<?php if (!$row->title) echo ' disabled="disabled"' ?> <?php echo ($row->title) ? 'onclick="window.open(\''. add_query_arg( array('form_id' => $id), $form_preview_link ) .'\', \'_blank\'); return false;"' : ''; ?>><?php _e('Preview', WDCFM()->prefix); ?></button>
				  </div>
				</div>
			...
			<div class="fm-theme-banner">
				<div class="fm-theme"  style="float:left; position: relative">
					<span><?php _e('Theme', WDCFM()->prefix); ?>:&nbsp;</span>
					...
					<a id="edit_css" class="pointer" onclick="window.open('<?php echo add_query_arg(array('current_id' => ($row->theme ? $row->theme : $default_theme), WDCFM()->nonce => wp_create_nonce(WDCFM()->nonce)), admin_url('admin.php?page=themes' . WDCFM()->menu_postfix . '&task=edit')); ?>'); return false;">
						<?php _e('Edit', WDCFM()->prefix); ?>
					</a>
          <br />
          <div id="old_theme_notice" style="display: none;"><div class="error inline"><p><?php _e('The theme you have selected is outdated. Please choose one from New Themes section.', WDCFM()->prefix); ?></p></div></div>
				</div>
				<div class="fm-page-actions">
				<?php if( $id ){ ?> 
					<a class="button button-primary" href="<?php echo $params['form_options_url']; ?>"><?php _e('Form Options', WDCFM()->prefix); ?></a>
					<a class="button" href="<?php echo $params['display_options_url']; ?>"><?php _e('Display Options', WDCFM()->prefix); ?></a>
				  <?php
					if ( !empty($params['advanced_layout_url']) ) {
					  ?>
          <a class="button" href="<?php echo $params['advanced_layout_url']; ?>"><?php _e('Advanced Layout', WDCFM()->prefix); ?></a>
					  <?php
					}
				}
				?>
			...
			<div class="meta-box-sortables" style="margin-top:30px;">
				<div id="postexcerpt" class="postbox closed" >
					<button type="button" class="button-link handlediv" aria-expanded="true">
						<span class="screen-reader-text"><?php _e('Toggle panel: Form Header', WDCFM()->prefix); ?></span>
						<span class="toggle-indicator" aria-hidden="true"></span>
					</button>
					<h2 class="hndle"><span><?php _e('Form Header', WDCFM()->prefix); ?></span></h2>
					<div class="inside">
						<div id="fm-header-content" class="panel-content">
							<div>
								<div class="fm-row">
									<label><?php _e('Title:', WDCFM()->prefix); ?></label>
								...
								<div class="fm-row">
									<label><?php _e('Description:', WDCFM()->prefix); ?></label>
							...
							<div>
								<div class="fm-row">
									<label><?php _e('Image:', WDCFM()->prefix); ?></label>
									...
									<button class="button add-button medium" onclick="fmOpenMediaUploader(event); return false;"><?php _e('Add Image', WDCFM()->prefix); ?></button>
								...
								<div class="fm-row">
									<label><?php _e('Image Animation:', WDCFM()->prefix); ?></label>
								...
								<div class="fm-row">
									<label for="header_hide_image"><?php _e('Hide Image on Mobile:', WDCFM()->prefix); ?></label>
			...
			<div class="fm-edit-content">
				<div class="fm-drag-and-drop">
					<div>
						<label for="enable_sortable"><?php _e('Enable Drag & Drop', WDCFM()->prefix); ?></label>
					...
					<div class="wdform_column ui-sortable" id="add_field_cont">
						...
						<div id="add_field" class="ui-sortable-handle">
							<div class="first-time-use">
								...
								<?php _e('Drag icon to the form to add a field.', WDCFM()->prefix); ?>
							</div>
  ...
  public function add_fields() {
    $pro_fields1 = array('file_upload', 'paypal', 'map');
    $pro_fields2 = array('file_upload', 'paypal', 'checkbox', 'radio', 'select', 'time_and_date', 'survey');
    $fields = array(
      __('BASIC FIELDS', WDCFM()->prefix) => array(
        array('type' => 'text', 'subtype' => 'text', 'title' => __('Single Line Text', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'textarea', 'title' => __('Paragraph Text', WDCFM()->prefix)),
        array('type' => 'survey', 'subtype' => 'spinner', 'title' => __('Number', WDCFM()->prefix)),
        array('type' => 'time_and_date', 'subtype' => 'date_new', 'title' => __('Date', WDCFM()->prefix)),
        array('type' => 'select', 'subtype' => 'own_select', 'title' => __('Select', WDCFM()->prefix)),
        array('type' => 'radio', 'subtype' => '', 'title' => __('Single Choice', WDCFM()->prefix)),
        array('type' => 'checkbox', 'subtype' => '', 'title' => __('Multiple Choice', WDCFM()->prefix)),
        array('type' => 'captcha', 'subtype' => 'recaptcha', 'title' => __('Recaptcha', WDCFM()->prefix)),
        array('type' => 'button', 'subtype' => 'submit_reset', 'title' => __('Submit', WDCFM()->prefix)),
	    	array('type' => 'captcha', 'subtype' => 'captcha', 'title' => __('Simple Captcha', WDCFM()->prefix)),
	  ),
      __('USER INFO FIELDS', WDCFM()->prefix) => array(
        array('type' => 'text', 'subtype' => 'name', 'title' => __('Name', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'submitter_mail', 'title' => __('Email', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'phone_new', 'title' => __('Phone', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'address', 'title' => __('Address', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'mark_map', 'title' => __('Mark on Map', WDCFM()->prefix)),
        array('type' => 'select', 'subtype' => 'country', 'title' => __('Country List', WDCFM()->prefix)),
        array('type' => 'time_and_date', 'subtype' => 'date_fields', 'title' => __('Date of Birth', WDCFM()->prefix)),
      ),
      __('LAYOUT FIELDS', WDCFM()->prefix) => array(
        array('type' => 'editor', 'subtype' => '', 'title' => __('HTML', WDCFM()->prefix)),
        array('type' => 'section_break', 'subtype' => '', 'title' => __('Section', WDCFM()->prefix)),
        array('type' => 'page_break', 'subtype' => '', 'title' => __('Page', WDCFM()->prefix)),
      ),
      __('ADVANCED', WDCFM()->prefix) => array(
        array('type' => 'file_upload', 'subtype' => '', 'title' => __('File Upload', WDCFM()->prefix)),
        array('type' => 'map', 'subtype' => '', 'title' => __('Map', WDCFM()->prefix)),
        array('type' => 'time_and_date', 'subtype' => 'time', 'title' => __('Time', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'send_copy', 'title' => __('Receive Copy', WDCFM()->prefix)),
        array('type' => 'time_and_date', 'subtype' => 'date_range', 'title' => __('Date Range', WDCFM()->prefix)),
        array('type' => 'survey', 'subtype' => 'star_rating', 'title' => __('Stars', WDCFM()->prefix)),
        array('type' => 'survey', 'subtype' => 'scale_rating', 'title' => __('Rating', WDCFM()->prefix)),
        array('type' => 'survey', 'subtype' => 'slider', 'title' => __('Slider', WDCFM()->prefix)),
        array('type' => 'survey', 'subtype' => 'range', 'title' => __('Range', WDCFM()->prefix)),
        array('type' => 'survey', 'subtype' => 'grading', 'title' => __('Grades', WDCFM()->prefix)),
        array('type' => 'survey', 'subtype' => 'matrix', 'title' => __('Table of Fields', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'hidden', 'title' => __('Hidden Input', WDCFM()->prefix)),
        array('type' => 'button', 'subtype' => 'button', 'title' => __('Custom Button', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'password', 'title' => __('Password', WDCFM()->prefix)),
        array('type' => 'text', 'subtype' => 'phone', 'title' => __('Phone-Area Code', WDCFM()->prefix)),
        array('type' => 'captcha', 'subtype' => 'arithmetic_captcha', 'title' => __('Arithmetic Captcha', WDCFM()->prefix)),
      ),
      __('PAYMENT', WDCFM()->prefix) => array(
        array('type' => 'paypal', 'subtype' => 'paypal_price_new', 'title' => __('Price', WDCFM()->prefix)),
        array('type' => 'paypal', 'subtype' => 'paypal_select', 'title' => __('Payment Select', WDCFM()->prefix)),
        array('type' => 'paypal', 'subtype' => 'paypal_radio', 'title' => __('Payment Single Choice', WDCFM()->prefix)),
        array('type' => 'paypal', 'subtype' => 'paypal_checkbox', 'title' => __('Payment Multiple Choice', WDCFM()->prefix)),
        array('type' => 'paypal', 'subtype' => 'paypal_shipping', 'title' => __('Shipping', WDCFM()->prefix)),
        array('type' => 'paypal', 'subtype' => 'paypal_total', 'title' => __('Total', WDCFM()->prefix)),
        array('type' => 'paypal', 'subtype' => 'stripe', 'title' => __('Stripe', WDCFM()->prefix)),
      ),
    );
    ob_start();
    ?>
    <div class="add-popup js">
      <div class="popup-header">
        <span class="popup-title">
          <?php _e('Add field', WDCFM()->prefix); ?>
        </span>
        <span title="<?php _e('Close', WDCFM()->prefix); ?>" alt="<?php _e('Close', WDCFM()->prefix); ?>" class="close-popup dashicons dashicons-no-alt" onclick="close_window()"></span>
      </div>
      <div class="popup-body meta-box-sortables">
        <div class="popup-body-col field_types">
		  <div class="field_types_cont">
			<h2 class="hndle field-types-filter_header"><span><?php _e('FIELD TYPES', WDCFM()->prefix); ?></span></h2>
			<span class="field-types-filter-cont">
            	<input class="field-types-filter" value="" placeholder="<?php _e('Filter', WDCFM()->prefix); ?>" tabindex="-1" type="search" />
            </span>
			<div class="postbox filtered-fields hide">
              ...
              <h2 class="hndle">
                <span><?php _e('Filtered fields', WDCFM()->prefix); ?></span>
              </h2>
            ...
            foreach ($fields as $section => $field) {
              ?>
              <div class="postbox<?php echo $section != __('BASIC FIELDS', WDCFM()->prefix) ? " closed" : ""; ?>">
                <button class="button-link handlediv" type="button" aria-expanded="true">
                  <span class="screen-reader-text"><?php echo __('Toggle panel:', WDCFM()->prefix) , $section; ?></span>
                ...
                <div class="inside">
                  ...
                  foreach ($field as $button) {
                    ?>
                    <button class="<?php echo ((WDCFM()->is_free == 1 && in_array($button['type'], $pro_fields1)) || (WDCFM()->is_free == 2 && in_array($button['type'], $pro_fields2))) ? 'wd-pro-fields ' : ''; ?>wd-button button-secondary" onclick="addRow(event, this, '<?php echo $button['type']; ?>', '<?php echo $button['subtype']; ?>'); return false;" data-type="type_<?php echo $button['subtype'] ? $button['subtype'] : $button['type']; ?>">
    ...
    wp_add_inline_script('jquery-ui-datepicker', WDW_FMC_Library::localize_ui_datepicker());
		...
		$form_attr = array(
			'id' => 'adminForm',
			'name' => 'adminForm',
			'class' => WDCFM()->css_prefix . 'form_options wd-form',
			'current_id' => $id,
  ...
  <div class="submenu-box">
    <div class="submenu-pad">
      <ul id="submenu" class="configuration">
        <li>
          <a id="general" class="fm_fieldset_tab" onclick="form_maker_options_tabs('general')" href="#"><?php _e('General Options', WDCFM()->prefix); ?></a>
        </li>
        <li>
          <a id="emailTab" class="fm_fieldset_tab" onclick="form_maker_options_tabs('emailTab')" href="#"><?php _e('Email Options', WDCFM()->prefix); ?></a>
        </li>
        <li>
          <a id="actions" class="fm_fieldset_tab" onclick="form_maker_options_tabs('actions')" href="#"><?php _e('Actions after Submission', WDCFM()->prefix); ?></a>
        </li>
        <li>
          <a id="payment" class="fm_fieldset_tab" onclick="form_maker_options_tabs('payment')" href="#"><?php _e('Payment Options', WDCFM()->prefix); ?></a>
        </li>
        <li>
          <a id="javascript" class="fm_fieldset_tab" onclick="form_maker_options_tabs('javascript'); codemirror_for_javascript();" href="#"><?php _e('JavaScript', WDCFM()->prefix); ?></a>
        </li>
        <li>
          <a id="conditions" class="fm_fieldset_tab" onclick="form_maker_options_tabs('conditions')" href="#"><?php _e('Conditional Fields', WDCFM()->prefix); ?></a>
        </li>
        <li>
          <a id="mapping" class="fm_fieldset_tab" onclick="form_maker_options_tabs('mapping')" href="#"><?php _e('MySQL Mapping', WDCFM()->prefix); ?></a>
        </li>
        <?php
...
<div>
      ...
      <div class="wd-table-col wd-table-col-50 wd-table-col-left">
        <div class="wd-box-section">
          <div class="wd-box-content">
						<span class="wd-group">
						  <label class="wd-label"><?php _e('Published', WDCFM()->prefix); ?></label>
						  <input type="radio" name="published" <?php echo $row->published == 1 ? 'checked="checked"' : '' ?> id="fm_go-published-1" class="wd-radio" value="1">
						  <label class="wd-label-radio" for="fm_go-published-1"><?php _e('Yes', WDCFM()->prefix); ?></label>
						  <input type="radio" name="published" <?php echo $row->published == 0 ? 'checked="checked"' : '' ?> id="fm_go-published-0" class="wd-radio" value="0">
						  <label class="wd-label-radio" for="fm_go-published-0"><?php _e('No', WDCFM()->prefix); ?></label>
						</span>
            <span class="wd-group">
						  <label class="wd-label"><?php _e('Save data(to database)', WDCFM()->prefix); ?></label>
            ...
            <span class="wd-group theme-wrap">
							...
							<select id="theme" name="theme" onChange="set_theme()">
								<optgroup label="<?php _e('New Themes', WDCFM()->prefix); ?>">
								  <option value="0" <?php echo $row->theme && $row->theme == 0 ? 'selected' : '' ?> data-version="2"><?php _e('Inherit From Website Theme', WDCFM()->prefix); ?></option>
                  <?php
                  $optiongroup = TRUE;
                  foreach ($themes
                  ...
                  ?>
									...
									<optgroup label="<?php _e('Outdated Themes', WDCFM()->prefix); ?>">
									<?php } ?>
                    <option value="<?php echo $theme->id; ?>" <?php echo(($theme->id == $row->theme) ? 'selected' : ''); ?> data-version="<?php echo $theme->version; ?>"><?php echo $theme->title; ?></option>
                    <?php } ?>
								</optgroup>
                ...
                <span class="wd-group">
								...
								<ul id="stats_fields">
									<li>
										...
										<label for="all_stats_fields"><?php _e('Select All', WDCFM()->prefix); ?></label>
									</li>
                  <?php
  ...
  <div id="emailTab_fieldset" class="adminform fm_fieldset_deactive js">
    <div class="wd-table">
      <div class="wd-table-col wd-table-col-100">
        <div class="wd-box-section">
          <div class="wd-box-content">
            <div class="wd-group">
              <label class="wd-label"><?php _e('Send E-mail', WDCFM()->prefix); ?></label>
    ...
    <div class="wd-table meta-box-sortables" id="fm_email_options">
      <div class="wd-table-col wd-table-col-50 wd-table-col-left">
        <div class="wd-box-section">
          <div class="wd-box-title">
            <strong><?php _e('Email to Administrator', WDCFM()->prefix); ?></strong>
          </div>
          <div class="wd-box-content">
            <div class="wd-group">
              <label class="wd-label" for="mailToAdd"><?php _e('Email to send submissions to', WDCFM()->prefix); ?></label>
              ...
              <span class="dashicons dashicons-plus-alt" title="<?php _e('Add more emails', WDCFM()->prefix); ?>" onclick="if (fm_check_email('mailToAdd')) {return false;};fm_add_submission_email('mail', 'mailToAdd', 'cfm_mail_div', '<?php echo WDCFM()->plugin_url; ?>')"></span>
              <div id="cfm_mail_div">
                <?php
                ...
                foreach ( $mail_array as $mail ) {
                  if ( $mail && $mail != ',' ) {
                    ...
                    <p class="fm_mail_input">
                      <span class="mail_name"><?php echo $mail; ?></span><span class="dashicons dashicons-trash" onclick="fm_delete_mail(this, '<?php echo $mail; ?>')" title="<?php _e('Delete Email', WDCFM()->prefix); ?>"></span>
                    </p>
                    <?php
                  }
                }
            ...
            <div class="postbox closed">
              <button class="button-link handlediv" type="button" aria-expanded="true">
                <span class="screen-reader-text"><?php _e('Toggle panel:', WDCFM()->prefix); ?></span>
                <span class="toggle-indicator" aria-hidden="false"></span>
              </button>
              <h2 class="hndle">
                <span><?php _e('Advanced', WDCFM()->prefix); ?></span>
              </h2>
              <div class="inside">
                <div class="wd-group">
                  <label class="wd-label"><?php _e('Email From', WDCFM()->prefix); ?></label>
                  <?php
                  $is_other = TRUE;
                  ...
                  <label style="<?php echo ($fields_count == 1) ? 'display:none;' : ''; ?>" class="wd-label-radio" for="other"><?php _e('Other', WDCFM()->prefix); ?></label>
                ...
                <div class="wd-group">
                  ...
                  <label style="<?php echo ($fields_count == 1) ? 'display: none;' : ''; ?>" class="wd-label-radio" for="other1"><?php _e('Other', WDCFM()->prefix); ?></label>
      ...
      <div class="wd-table-col wd-table-col-50 wd-table-col-right">
        <div class="wd-box-section">
          <div class="wd-box-title">
            <strong><?php _e('Email to User', WDCFM()->prefix); ?></strong>
          </div>
          <div class="wd-box-content">
            <div class="wd-group">
              <label class="wd-label"><?php _e('Send to', WDCFM()->prefix); ?></label>
              <?php
              $fields = explode('*:*id*:*type_submitter_mail*:*type*:*', $row->form_fields);
              $fields_count = count($fields);
              if ( $fields_count == 1 ) {
                _e('There is no email field', WDCFM()->prefix);
              }
              else {
                for ( $i = 0; $i < $fields_count - 1; $i++ ) {
                  ?>
            ...
            <div class="postbox closed">
              ...
              <div class="inside">
                ...
                <div class="wd-group" <?php echo($row->mail_verify == 0 ? 'style="display:none;"' : '') ?> id="expire_link">
                  ...
                  <small><?php _e(' -- hours (0 - never expires).', WDCFM()->prefix); ?></small>
    ...
    <div class="wd-table">
      <div class="wd-table-col-70">
        <div class="wd-box-section">
          <div class="wd-box-content">
						<span class="wd-group">
							<label class="wd-label"><?php _e('Action type', WDCFM()->prefix); ?></label>
							...
							<label for="text_type_none"><?php _e('Stay on Form', WDCFM()->prefix); ?></label>
							<br>
							...
							<label for="text_type_post"><?php _e('Post', WDCFM()->prefix); ?></label>
							<br>
							...
							<label for="text_type_page"><?php _e('Page', WDCFM()->prefix); ?></label>
							<br>
							...
							<label for="text_type_custom_text"><?php _e('Custom Text', WDCFM()->prefix); ?></label>
							<br>
							...
							<label for="text_type_url"><?php _e('URL', WDCFM()->prefix); ?></label>
						</span>
            <span class="wd-group">
							<div id="post" <?php echo(($row->submit_text_type != 2) ? 'style="display:none"' : ''); ?>>
								<label class="wd-label"><?php _e('Post', WDCFM()->prefix); ?></label>
                  ...
                  ?>
							...
							<div id="page" <?php echo(($row->submit_text_type != 5) ? 'style="display:none"' : ''); ?>>
								<label class="wd-label"><?php _e('Page', WDCFM()->prefix); ?></label>
                  ...
                  ?>
							...
							<div id="custom_text" <?php echo(($row->submit_text_type != 3) ? 'style="display: none;"' : ''); ?>>
								<label class="wd-label"><?php _e('Text', WDCFM()->prefix); ?></label>
								<div class="fm_textarea-buttons">
                ...
                ?>
							...
							<div id="url" <?php echo(($row->submit_text_type != 4) ? 'style="display:none"' : ''); ?>>
								<label class="wd-label"><?php _e('URL', WDCFM()->prefix); ?></label>
    ...
    <div class="wd-table">
      <div class="wd-table-col-70">
        <div class="wd-box-section">
          <div class="wd-box-content">
            <div class="wd-group">
              <label class="wd-label"><?php _e('Payment Method', WDCFM()->prefix); ?></label>
              ...
              <label for="paypal_mode0"><?php _e('None', WDCFM()->prefix); ?></label><br>
              ...
              <label for="paypal_mode1"><?php _e('Paypal', WDCFM()->prefix); ?></label><br>
              ...
              <label for="paypal_mode2"><?php _e('Stripe', WDCFM()->prefix); ?></label>
            </div>
            <div class="fm_payment_option">
              <div class="wd-group">
                <label class="wd-label" for="payment_currency"><?php _e('Payment Currency', WDCFM()->prefix); ?></label>
              ...
              <div class="wd-group">
                ...
                <label class="wd-label-radio" for="checkout_mode-1"><?php _e('Production', WDCFM()->prefix); ?></label>
                ...
                <label class="wd-label-radio" for="checkout_mode-0"><?php _e('Testmode', WDCFM()->prefix); ?></label>
              </div>
              <div class="wd-group">
            ...
            <div class="fm_payment_option_stripe">
              ...
              if ( $stripe_addon['enable'] && !empty($stripe_addon['html']) ) {
                ...
                $stripe_add_on_link = '<a href="https://web-dorado.com/products/wordpress-form/add-ons/stripe.html" target="_blank">' . __('Form Maker Stripe Integration', WDCFM()->prefix) . '</a>';
                echo '<div class="error inline"><p>' . sprintf(__("Please install %s add-on to use this feature.", WDCFM()->prefix), $stripe_add_on_link) . '</p></div>';
              }
              ?>
            </div>
          </div>
                  ...
                  <div id="condition<?php echo $k; ?>" class="fm_condition">
                    <div id="conditional_fileds<?php echo $k; ?>">
                      <select id="show_hide<?php echo $k; ?>" name="show_hide<?php echo $k; ?>" class="fm_condition_show_hide">
                        ...
                        } ?>><?php _e('Show', WDCFM()->prefix); ?></option>
                        <option value="0" <?php if ( $show_hide[$k] == 0 ) {
                          echo 'selected="selected"';
                        } ?>><?php _e('Hide', WDCFM()->prefix); ?></option>
                      </select>
                      ...
                      <select id="all_any<?php echo $k; ?>" name="all_any<?php echo $k; ?>" class="fm_condition_all_any">
                        <option value="and" <?php if ( $all_any[$k] == "and" ) {
                          echo 'selected="selected"';
                        } ?>><?php _e('all', WDCFM()->prefix); ?></option>
                        <option value="or" <?php if ( $all_any[$k] == "or" ) {
                          echo 'selected="selected"';
                        } ?>><?php _e('any', WDCFM()->prefix); ?></option>
                      </select>
                      <span style="display: inline-block; width: 100%; max-width: 235px;"><?php _e('of the following match:', WDCFM()->prefix); ?></span>
  ...
  <div id="mapping_fieldset" class="adminform fm_fieldset_deactive">
    <?php
    if ( WDCFM()->is_demo ) {
      echo WDW_FMC_Library::message_id(0, 'This feature is disabled in demo.', 'error');
    }
    else {
    ?>
    <div class="wd-table">
            ...
            <div class="wd-group">
              <div style="text-align: right; padding-bottom: 20px;">
                <button id="add_query" class="wd-button button-primary" onclick="tb_show('', '<?php echo add_query_arg(array(
                                                                                                                         ...
                                                                                                                         'height' => '500',
                                                                                                                         'TB_iframe' => '1'
                                                                                                                       ), admin_url('admin-ajax.php')); ?>'); return false;"><?php _e('Add Query', WDCFM()->prefix); ?></button>
              </div>
              <?php if ( $queries ) { ?>
                <table class="wp-list-table widefat fixed posts table_content">
                  <thead>
                  <tr>
                    <th style="width:86%;" class="table_large_col"><?php _e('Query', WDCFM()->prefix); ?></th>
                    <th style="width:14%;" class="table_large_col"><?php _e('Delete', WDCFM()->prefix); ?></th>
                  </tr>
                  </thead>
                  <?php
	...
	*/
	public function form_layout( $params ) {
		...
		// Pass the content to form.
		$form_attr = array(
			'id' => WDCFM()->css_prefix . 'ApplyLayoutForm',
			'name' => 'adminForm',
			'class' => WDCFM()->css_prefix . 'advanced_layout wd-form',
			'current_id' => $id,
	...
	*/
	private function body_form_layout( $params ) { 
		...
		<div class="wd-table">
			<div class="wd-table-col-100">
				<div class="wd-box-section">						  
					<div class="wd-box-content">
						<p><?php _e('To customize the layout of the form fields uncheck the Auto-Generate Layout box and edit the HTML.', WDCFM()->prefix); ?></p>
						<p><?php _e('You can change positioning, add in-line styles and etc. Click on the provided buttons to add the corresponding field.', WDCFM()->prefix); ?></p>
						<p><?php _e('This will add the following line:', WDCFM()->prefix); ?>
						<b><span class="cm-tag">&lt;div</span> <span class="cm-attribute">wdid</span>=<span class="cm-string">"example_id"</span> <span class="cm-attribute">class</span>=<span class="cm-string">"wdform_row"</span><span class="cm-tag">&gt;</span>%example_id - Example%<span class="cm-tag">&lt;/div&gt;</span></b>	, where <b><span class="cm-tag">&lt;div&gt;</span></b> <?php _e('is used to set a row.', WDCFM()->prefix); ?></p>
						<p>
							<b style="color:red"><?php _e('Notice', WDCFM()->prefix); ?></b><br>
							<?php _e('Make sure not to publish the same field twice. This will cause malfunctioning of the form.', WDCFM()->prefix); ?>
						</p>
						<div class="wd-group">
							<label class="wd-label autogen_layout_label" for="autogen_layout"><?php _e('Auto Generate Layout?', WDCFM()->prefix); ?></label>
						...
						<div class="wd-group">
							...
							<span class="button button-hero fm_auto_format_button" onclick="autoFormat()"><strong><?php _e('Apply Source Formatting', WDCFM()->prefix); ?></strong> <em>(<?php _e('ctrl-enter', WDCFM()->prefix); ?>)</em></span>
	...
	public function display_options($params) {
		...
		$form_attr = array(
			'id' => 'adminForm',
			'name' => 'adminForm',
			'class' => WDCFM()->prefix . '_display_options wd-form',
			'current_id' => $id,
			'action' => add_query_arg( array('page' => $page, 'current_id' => $id ), $page_url),
		);
		echo $this->form(ob_get_clean(), $form_attr);
						...
						<div class="wd-box-section">
							<div class="wd-box-content">
											...
											<div class="fm-row fm-form-types">
												<label  class="wd-label"><?php  _e('Form Type', WDCFM()->prefix); ?></label>
												<label>
						...
						<div class="wd-box-section">
							<div class="wd-box-content">
								<span class="wd-group fm-embedded <?php echo $row->type == 'embedded' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Form Placement', WDCFM()->prefix); ?></label>
									<div id="fm-embedded-element">
									<p><?php  _e('Use', WDCFM()->prefix); ?></p>
									...
									<p><?php  _e('shortcode to display the form.', WDCFM()->prefix); ?></p>
									</div>
								</span>
								<span class="wd-group fm-popover <?php echo $row->type == 'popover' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Animation Effect', WDCFM()->prefix); ?></label>
								...
								<span class="wd-group fm-popover <?php echo $row->type != 'popover' ? 'fm-hide' : 'fm-show'; ?>">
									<label class="wd-label"><?php  _e('Loading Delay', WDCFM()->prefix); ?></label>
									<input type="number" name="popover_loading_delay" value="<?php echo $row->popover_loading_delay; ?>" /> seconds
									<div>
										<?php  _e('Define the amount of time before the form popup appears after the page loads.', WDCFM()->prefix); ?>
										<?php  _e('Set 0 for no delay.', WDCFM()->prefix); ?>
									</div>
								</span>
								<span class="wd-group fm-popover <?php echo $row->type == 'popover' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Frequency', WDCFM()->prefix); ?></label>
									<input type="number" name="popover_frequency" value="<?php echo $row->popover_frequency; ?>" /> days
									<div>
										<?php  _e('Display the popup to the same visitor (who has closed the popup/submitted the form) after this period.', WDCFM()->prefix); ?>
										<?php  _e('Set the value to 0 to always show.', WDCFM()->prefix); ?>
									</div>
								</span>
								<span class="wd-group fm-topbar <?php echo $row->type == 'topbar' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Position', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-topbarpos-1"><?php  _e('Top', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-topbarpos-0"><?php  _e('Bottom', WDCFM()->prefix); ?></label>
								</span>
								<span class="wd-group fm-topbar topbar_remain_top <?php echo $row->type != 'topbar' ? 'fm-hide' : ($row->topbar_position == 1 ? 'fm-show' : 'fm-hide') ?>">
									<label class="wd-label"><?php  _e('Remain at top when scrolling', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-remaintop-1"><?php  _e('Yes', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-remaintop-0"><?php  _e('No', WDCFM()->prefix); ?></label>
								</span>
								<span class="wd-group fm-topbar <?php echo $row->type == 'topbar' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Allow Closing the bar', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-topbarclosing-1"><?php  _e('Yes', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-topbarclosing-0"><?php  _e('No', WDCFM()->prefix); ?></label>
								</span>
								<span class="wd-group fm-topbar topbar_hide_duration <?php echo $row->type != 'topbar' ? 'fm-hide' : 'fm-show' ?>">
									...
									<div>
										<?php  _e('Display the topbar to the same visitor (who has closed the popup/submitted the form) after this period.', WDCFM()->prefix); ?>
										<?php  _e('Set the value to 0 to always show.', WDCFM()->prefix); ?>
									</div>
								</span>
								<span class="wd-group fm-scrollbox <?php echo $row->type == 'scrollbox' ? 'fm-show' : 'fm-hide' ?>">
									...
									<label class="wd-label-radio" for="fm_do-scrollboxposition-0"><?php  _e('Left', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-scrollboxposition-1"><?php  _e('Right', WDCFM()->prefix); ?></label>
								</span>
								<span class="wd-group fm-scrollbox <?php echo $row->type != 'scrollbox' ? 'fm-hide' : 'fm-show'; ?>">
									...
									<div>
										<?php  _e('Define the amount of time before the form scrollbox appears after the page loads. Set 0 for no delay.', WDCFM()->prefix); ?>
									</div>
								</span>
								<span class="wd-group fm-scrollbox <?php echo $row->type == 'scrollbox' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Frequency', WDCFM()->prefix); ?></label>
									...
									<div>
										<?php  _e('Display the scrollbox to the same visitor (who has closed the popup/submitted the form) after this period.', WDCFM()->prefix); ?>
										<?php  _e('Set the value to 0 to always show.', WDCFM()->prefix); ?>
									</div>
								</span>
								<span class="wd-group fm-popover fm-topbar fm-scrollbox <?php echo $row->type != 'embedded' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Always show for administrator', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-showforadmin-1"><?php  _e('Yes', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-showforadmin-0"><?php  _e('No', WDCFM()->prefix); ?></label>
									<div>
										<?php  _e('If this option is enabled, website admins will always see the form.', WDCFM()->prefix); ?>
									</div>
								</span>
								<span class="wd-group fm-scrollbox <?php echo $row->type == 'scrollbox' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Trigger Point', WDCFM()->prefix); ?></label>
									<input type="number" name="scrollbox_trigger_point" value="<?php echo $row->scrollbox_trigger_point; ?>"/>%
									<div>
										<?php  _e('Set the percentage of the page height, where the scrollbox form will appear after scrolling down.', WDCFM()->prefix); ?>
									</div>
								</span>
								<span class="wd-group fm-scrollbox <?php echo $row->type == 'scrollbox' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Allow Closing the bar', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-scrollboxclosing-1"><?php  _e('Yes', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-scrollboxclosing-0"><?php  _e('No', WDCFM()->prefix); ?></label>
								</span>
								<span class="wd-group fm-scrollbox <?php echo $row->type == 'scrollbox' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Allow Minimize', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-scrollboxminimize-1"><?php  _e('Yes', WDCFM()->prefix); ?></label>
								...
								<span class="wd-group fm-scrollbox <?php echo $row->type == 'scrollbox' ? 'fm-show' : 'fm-hide' ?>">
									...
									<label class="wd-label-radio" for="fm_do-scrollboxautohide-1"><?php  _e('Yes', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-scrollboxautohide-0"><?php  _e('No', WDCFM()->prefix); ?></label>
									<div>
										<?php  _e('Hide the scrollbox form again when visitor scrolls back up.', WDCFM()->prefix); ?>
									</div>
								</span>
								<span class="wd-group fm-popover fm-topbar fm-scrollbox <?php echo $row->type != 'embedded' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Display on', WDCFM()->prefix); ?></label>
								...
								<span class="wd-group fm-popover fm-topbar fm-scrollbox fm-cat-show <?php echo $row->type != 'embedded' && $show_cats ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e("Display on these categories' posts", WDCFM()->prefix); ?></label>
								...
								<span class="wd-group fm-popover fm-topbar fm-scrollbox fm-posts-show <?php echo (in_array('everything', $selected_types) || in_array('post', $selected_types)) && $row->type != 'embedded' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Display on these posts', WDCFM()->prefix); ?></label>
									<div class="fm-mini-heading">
										<?php  _e('Click on input area to view the list of posts.', WDCFM()->prefix); ?>
										<?php  _e('If left empty the form will appear on all posts.', WDCFM()->prefix); ?>
									</div>
									<p><?php  _e('Posts defined below will override all settings above.', WDCFM()->prefix); ?></p>
									<ul class="fm-pp">
								...
								<span class="wd-group fm-popover fm-topbar fm-scrollbox fm-pages-show <?php echo (in_array('everything', $selected_types) || in_array('page', $selected_types)) && $row->type != 'embedded' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Display on these pages', WDCFM()->prefix); ?></label>
									<div class="fm-mini-heading">
										<?php  _e('Click on input area to view the list of pages. ', WDCFM()->prefix); ?>
										<?php  _e('If left empty the form will appear on all pages.', WDCFM()->prefix); ?>
									</div>
									<p><?php  _e('Pages defined below will override all settings above.', WDCFM()->prefix); ?></p>
									<ul class="fm-pp">
								...
								<span class="wd-group fm-popover fm-topbar fm-scrollbox <?php echo $row->type != 'embedded' ? 'fm-show' : 'fm-hide' ?>">
									<label class="wd-label"><?php  _e('Hide on Mobile', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-hidemobile-1"><?php  _e('Yes', WDCFM()->prefix); ?></label>
									...
									<label class="wd-label-radio" for="fm_do-hidemobile-0"><?php  _e('No', WDCFM()->prefix); ?></label>
								</span>
							</div>
						</div>
					</div>
