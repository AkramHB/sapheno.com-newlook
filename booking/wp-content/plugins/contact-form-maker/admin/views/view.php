...
class FMAdminView_fmc {
  ...
  protected function form($content = '', $attr = array()) {
    ...
    $name = isset($attr['name']) ? esc_attr($attr['name']) : WDCFM()->prefix . '_form';
    $id = isset($attr['id']) ? esc_attr($attr['id']) : '';
    $class = isset($attr['class']) ? esc_attr($attr['class']) : WDCFM()->prefix . '_form';
    ...
    ?>
      ...
      echo $content;
      // Add nonce to form.
      wp_nonce_field(WDCFM()->nonce, WDCFM()->nonce);
        ?>
  ...
  protected function search() {
    ...
    <p class="search-box">
      ...
      <input class="button" value="<?php _e('Search', WDCFM()->prefix); ?>" type="button" onclick="search(this)" />
    </p>
    <?php
  ...
  protected function pagination($page_url, $total, $items_per_page = 20) {
    $page_number = WDW_FMC_Library::get('paged', 1);
    $search = WDW_FMC_Library::get('s', '');
    $orderby = WDW_FMC_Library::get('orderby', '');
    $order = WDW_FMC_Library::get('order', '');
	$url_arg = array();
	if( !empty($search) ) {
		$url_arg['s'] = $search;
	}
    ...
    <div class="tablenav-pages">
      <span class="displaying-num">
        <?php printf(_n('%s item', '%s items', $total, WDCFM()->prefix), $total); ?>
      </span>
      <?php
      if ( $total > $items_per_page ) {
        ?>
      <span class="pagination-links" data-pages-count="<?php echo $pages_count; ?>">
        ...
        if ( $page_number == 1 ) {
          ...
          ?>
          <a href="<?php echo add_query_arg(array('paged' => 1), $page_url); ?>" class="first-page"><span class="screen-reader-text"><?php _e('First page', WDCFM()->prefix); ?></span><span aria-hidden="true">«</span></a>
          <a href="<?php echo add_query_arg(array('paged' => ($page_number == 1 ? 1 : ($page_number - 1))), $page_url); ?>" class="previous-page"><span class="screen-reader-text"><?php _e('Previous page', WDCFM()->prefix); ?></span><span aria-hidden="true">‹</span></a>
          <?php
        }
        ?>
        <span class="paging-input">
          <label for="current-page-selector" class="screen-reader-text"><?php _e('Current Page', WDCFM()->prefix); ?></label>
        ...
        if ( $page_number >= $pages_count ) {
          ...
          ?>
          <a href="<?php echo add_query_arg(array('paged' => ($page_number >= $pages_count ? $pages_count : ($page_number + 1))), $page_url); ?>" class="next-page"><span class="screen-reader-text"><?php _e('Next page', WDCFM()->prefix); ?></span><span aria-hidden="true">›</span></a>
          <a href="<?php echo add_query_arg(array('paged' => $pages_count), $page_url); ?>" class="last-page"><span class="screen-reader-text"><?php _e('Last page', WDCFM()->prefix); ?></span><span aria-hidden="true">»</span></a>
          <?php
        }
        ?>
      </span>
  ...
  protected function bulk_actions($actions) {
    ...
    <div class="alignleft actions bulkactions">
      <label for="bulk-action-selector-top" class="screen-reader-text"><?php _e('Select bulk action', WDCFM()->prefix); ?></label>
      <select name="bulk_action" id="bulk-action-selector-top">
        <option value="-1"><?php _e('Bulk Actions', WDCFM()->prefix); ?></option>
        <?php
      ...
      <input type="button" id="doaction" class="button action" onclick="wd_bulk_action(this)" value="<?php _e('Apply', WDCFM()->prefix); ?>" />
    </div>
    <?php
