  ...
  public function blocked_ips() {
	...
	*/
	public function get_forms() {
		global $wpdb;
		$query = "SELECT `id`, `title`, `published` FROM " . $wpdb->prefix . "formmaker " . (!WDCFM()->is_free ? '' : ' WHERE id' . (WDCFM()->is_free == 1 ? ' NOT ' : ' ') . 'IN (' . (get_option('contact_form_forms', '') != '' ? get_option('contact_form_forms') : 0) . ')') . " ORDER BY `title`";
  ...
  */
  ...
  public function view_for_star_rating( $element_value, $element_label ) {
    ...
    for ( $j = 0; $j < $new_filename[1]; $j++ ) {
      $stars .= '<img id="' . $element_label . '_star_' . $j . '" src="' . WDCFM()->plugin_url . '/images/star_' . $new_filename[2] . '.png?ver=' . WDCFM()->plugin_version . '" /> ';
    }
    for ( $k = $new_filename[1]; $k < $new_filename[0]; $k++ ) {
      $stars .= '<img id="' . $element_label . '_star_' . $k . '" src="' . WDCFM()->plugin_url . '/images/star.png?ver=' . WDCFM()->plugin_version . '" /> ';
    }
  ...
  public function images_for_star_rating( $element_value, $label_id ) {
    ...
    for ( $j = 0; $j < $stars_value[0]; $j++ ) {
      $edit_stars .= '<img id="' . $label_id . '_star_' . $j . '" onclick="edit_star_rating(' . $j . ',' . $label_id . ')" src="' . WDCFM()->plugin_url . '/images/star_yellow.png?ver=' . WDCFM()->plugin_version . '" /> ';
    }
    for ( $k = $stars_value[0]; $k < $stars_value[1]; $k++ ) {
      $edit_stars .= '<img id="' . $label_id . '_star_' . $k . '" onclick="edit_star_rating(' . $k . ',' . $label_id . ')" src="' . WDCFM()->plugin_url . '/images/star.png?ver=' . WDCFM()->plugin_version . '" /> ';
    }
