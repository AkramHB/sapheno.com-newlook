...
class FMModelForm_maker_fmc {
  ...
  public function showform( $id, $type = 'embedded' ) {
    global $wpdb;
    $row = $wpdb->get_row($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'formmaker WHERE id="%d"' . (!WDCFM()->is_free ? '' : ' AND id' . (WDCFM()->is_free == 1 ? ' NOT ' : ' ') . 'IN (' . (get_option('contact_form_forms', '') != '' ? get_option('contact_form_forms') : 0) . ')'), $id));

    if ( !$row ) {
      echo WDW_FMC_Library::message(__('Something went wrong when trying to display this form.', WDCFM()->prefix), 'fm-notice-error');
      return FALSE;
    }

    if ( $row->type != $type ) {
      echo WDW_FMC_Library::message(__('The form you are trying to view does not have Embedded display type.', WDCFM()->prefix), 'fm-notice-error');
      return FALSE;
    }

    $form_preview = (WDW_FMC_Library::get('form_id', '') == $id) ? TRUE : FALSE;
    ...
    $theme_id = WDW_FMC_Library::get('test_theme', '');
  ...
  public function create_css( $theme_id, $form_theme, $old = TRUE, $force_rewrite = FALSE ) {
    ...
    if ( !is_dir($wp_upload_dir['basedir'] . $frontend_dir . 'css') ) {
      ...
      file_put_contents($wp_upload_dir['basedir'] . $frontend_dir . 'css/index.html', WDW_FMC_Library::forbidden_template());
    }
    ...
    if ( !$old ) {
      ...
      $css_content .= '.fm-form-container.fm-theme' . $theme_id . ' .fm-form .file-picker {'.
						( !empty($form_theme['OPFBgUrl']) ? 'display: inline-block; width: 22px; height: 22px; background: url("' . WDCFM()->plugin_url . '/' . $form_theme['OPFBgUrl'] . '");' : '').
      ...
      $css_content .= '.fm-form-container.fm-theme' . $theme_id . ' .fm-form select {'.
        ( !empty($form_theme['IPBGColor']) ? 'background-color:' . $form_theme['IPBGColor'] . ';' : '').
							( !empty($form_theme['SBPBackground']) ? 'background-image: url("' . WDCFM()->plugin_url . '/' . $form_theme['SBPBackground'] . '");' : '').
      ...
      $css_content .= '.fm-form-container.fm-theme' . $theme_id . ' .fm-form .checkbox-div input[type="checkbox"]:checked + label span:after {'.
        ...
        ( !empty($form_theme['MCCPBGColor']) ? 'background-color:' . $form_theme['MCCPBGColor'] . ';' : '').
							( !empty($form_theme['MCCPBackground']) ? 'background-image: url("' . WDCFM()->plugin_url . '/' . $form_theme['MCCPBackground'] .'");' : '').
  ...
  public function savedata( $form, $id ) {
    ...
    if ( isset($_POST["counter" . $id]) ) {
      ...
      if ( (isset($_POST["save_or_submit" . $id]) && $_POST["save_or_submit" . $id] != 'save') ) {
        if ( isset($_POST["captcha_input"]) ) {
          ...
          if ( md5($captcha_input) == $session_wd_captcha_code ) {
            ...
            $_SESSION['massage_after_submit' . $id] = addslashes(addslashes(__('Error, incorrect Security code.', WDCFM()->prefix)));
                ...
                foreach ($jsonResponse['error-codes'] as $errorcode) {
                  switch ($errorcode) {
                    ...
                    case 'invalid-input-secret' : {
                      $_SESSION['massage_after_submit' . $id] = addslashes(addslashes(__('Error, incorrect secret code.', WDCFM()->prefix)));
                      break;
                    }
                    ...
                    case 'bad-request' :
                    default: {
                      $_SESSION['massage_after_submit' . $id] = addslashes(addslashes(__('Verification failed.', WDCFM()->prefix)));
                      break;
                    }
                  }
                }
      ...
      if ( $correct ) {
        ...
        if ( $blocked_ip ) {
          $_SESSION['massage_after_submit' . $id] = addslashes(__('Your ip is blacklisted. Please contact the website administrator.', WDCFM()->prefix));
  ...
  public function save_db( $counter, $id ) {
    ...
    $stripeToken = WDW_FMC_Library::get($stripe_post_key, '');
    ...
    if ( $save_uploads == 0 ) {
      ...
      foreach ( $all_files as &$all_file ) {
        ...
        if ( !move_uploaded_file($fileTemp, $destination . '/' . $fileName) ) {
          return array( 1, addslashes(__('Error, file cannot be moved.', WDCFM()->prefix)) );
        }
