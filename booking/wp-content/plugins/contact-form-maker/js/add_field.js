...
function fm_check_something_really_important() {
  ...
  if (is7 >= 9) {
    alert(form_maker.important_message);
    return true;
  }
  return false;
}
...
function enable2() {
  ...
  jQuery(".field_options").addClass("field_options-large");
}
...
function popup_ready() {
  ...
  jQuery(".field_options").removeClass("field_options-large");
...
function fm_add_field_button(that, subtype) {
  if (jQuery(that).hasClass('wd-pro-fields') && subtype != 'stripe') {
    ...
    jQuery('#add-button-cont').removeClass('add-button-cont').html('<div class="error"><p>' + form_maker.disabled1 + '</p><p><a href="https://web-dorado.com/files/fromFormMaker.php" target="_blank">' + form_maker.disabled2 + '</a></p></div>');
  }
  else {
...
function go_to_type_mark_map(new_id) {
  w_attr_name = [];
  w_attr_value = [];
  type_mark_map(new_id, 'Mark your place on map', '', 'top', 'no', '2.294254', '48.858334', "2.294254", "48.858334", "13", "400", "300", 'wdform_map', '', w_attr_name, w_attr_value);
}
...
function type_captcha(i,w_field_label, w_field_label_size, w_field_label_pos, w_hide_label, w_digit, w_class, w_attr_name, w_attr_value) {
  ...
  adding.setAttribute("src", url_for_ajax + "?action=formmakerwdcaptcha_fmc&digit=" + w_digit + "&i=form_id_temp");
...
function change_arithmetic_captcha(value, field) {
  ...
  arithmetic_captcha.setAttribute("src", url_for_ajax + "?action=formmakerwdmathcaptcha_fmc&operations_count=" + oper_count + "&operations=" + operations.replace('+', '@') + "&i=form_id_temp");
}
...
function type_arithmetic_captcha(i,w_field_label, w_field_label_size, w_field_label_pos, w_hide_label, w_count, w_operations, w_class, w_input_size, w_attr_name, w_attr_value) {
  ...
  adding.setAttribute("src", url_for_ajax + "?action=formmakerwdmathcaptcha_fmc&operations_count=" + w_count + "&operations=" + w_operations.replace("+", "@") + "&i=form_id_temp");
