...
class WDW_FMC_Library {
  ...
  public static function ordering($id, $orderby, $order, $text, $page_url, $additional_class = '') {
    ...
    <th id="<?php echo $id; ?>" class="<?php echo implode(' ', $class); ?>">
      <a href="<?php echo add_query_arg( array('orderby' => $id, 'order' => $order), $page_url ); ?>"
         title="<?php _e('Click to sort by this item', WDCFM()->prefix); ?>">
  ...
  public static function fm_redirect($url) {
    $url = html_entity_decode(wp_nonce_url($url, WDCFM()->nonce, WDCFM()->nonce));
    ?>
    <script>
  ...
  public static function create_js( $form_id, $force_rewrite = FALSE ) {
    ...
    if ( !is_dir($wp_upload_dir['basedir'] . $frontend_dir) ) {
      ...
      file_put_contents($wp_upload_dir['basedir'] . $frontend_dir . 'index.html', WDW_FMC_Library::forbidden_template());
    }
    if ( !is_dir($wp_upload_dir['basedir'] . $frontend_dir . 'js') ) {
      mkdir($wp_upload_dir['basedir'] . $frontend_dir . 'js');
      file_put_contents($wp_upload_dir['basedir'] . $frontend_dir . 'js/index.html', WDW_FMC_Library::forbidden_template());
    }
    ...
    $row_display = WDW_FMC_Library::display_options($form_id);
  ...
  if( Date.parse(jQuery("#wdform_' . $id1 . '_element' . $form_id . '").val() + " 23:59:59") < fm_currentDate.getTime() ) {
    alert("' . __('You cannot select former dates. Choose a date starting from the current one.', WDCFM()->prefix) . '");
    return false;
  }';
            }
  ...
  if(x.find(jQuery("div[wdid=' . $id1 . ']")).length != 0 && x.find(jQuery("div[wdid=' . $id1 . ']")).css("display") != "none") {
    if(jQuery("#wdform_' . $id1 . '_element_dollars' . $form_id . '").val() == "' . $w_title[0] . '" || jQuery("#wdform_' . $id1 . '_element_dollars' . $form_id . '").val() == "") {
      alert("' . addslashes($label . ' ' . __('field is required.', WDCFM()->prefix)) . '");
  ...
  if(x.find(jQuery("div[wdid=' . $id1 . ']")).length != 0 && x.find(jQuery("div[wdid=' . $id1 . ']")).css("display") != "none") {
    ...
    if(' . ($required ? 'true' : 'false') . ' || jQuery("#wdform_' . $id1 . '_element_dollars' . $form_id . '").val() != "' . $w_title[0] . '" || jQuery("#wdform_' . $id1 . '_element_cents' . $form_id . '").val() != "' . $w_title[1] . '") {
      if((range_max!=-1 && parseFloat(price)>range_max) || parseFloat(price)<range_min) {
        alert("' . addslashes((__('The', WDCFM()->prefix)) . $label . (__('value must be between', WDCFM()->prefix)) . ($param['w_range_min'] ? $param['w_range_min'] : 0) . '-' . ($param['w_range_max'] ? $param['w_range_max'] : "any")) . '");
        old_bg=x.find(jQuery("div[wdid=' . $id1 . ']")).css("background-color");
        x.find(jQuery("div[wdid=' . $id1 . ']")).effect( "shake", {}, 500 ).css("background-color","#FF8F8B").animate({backgroundColor: old_bg}, {duration: 500, queue: false });
        jQuery("#wdform_' . $id1 . '_element_dollars' . $form_id . '").focus();
        return false;
  ...
  jQuery("<input type=\"hidden\" name=\"wdform_' . $id1 . '_element_label' . $form_id . '\" />").val(jQuery("#wdform_' . $id1 . '_element' . $form_id . ' option:selected").text()).appendTo("#form' . $form_id . '");';
            $onsubmit_js .= '
  jQuery("<input type=\"hidden\" name=\"wdform_' . $id1 . '_element_quantity_label' . $form_id . '\" />").val("' . (__("Quantity", WDCFM()->prefix)) . '").appendTo("#form' . $form_id . '");';
  ...
  if(x.find(jQuery("div[wdid=' . $id1 . ']")).length != 0 && x.find(jQuery("div[wdid=' . $id1 . ']")).css("display") != "none") {
    if(parseInt(jQuery("#wdform_' . $id1 . '_sum_element' . $form_id . '").html()) > ' . $param['w_total'] . ') {
      alert("' . addslashes(__("Your score should be less than", WDCFM()->prefix)) . ' ' . $param['w_total'] . '");
      return false;
    }
  }';
            }
  ...
  public static function get_countries() {
    $countries = array(
      "" => "",
      "Afghanistan" => __("Afghanistan", WDCFM()->prefix),
      "Albania" => __("Albania", WDCFM()->prefix),
      "Algeria" => __("Algeria", WDCFM()->prefix),
      "Andorra" => __("Andorra", WDCFM()->prefix),
      "Angola" => __("Angola", WDCFM()->prefix),
      "Antigua and Barbuda" => __("Antigua and Barbuda", WDCFM()->prefix),
      "Argentina" => __("Argentina", WDCFM()->prefix),
      "Armenia" => __("Armenia", WDCFM()->prefix),
      "Australia" => __("Australia", WDCFM()->prefix),
      "Austria" => __("Austria", WDCFM()->prefix),
      "Azerbaijan" => __("Azerbaijan", WDCFM()->prefix),
      "Bahamas" => __("Bahamas", WDCFM()->prefix),
      "Bahrain" => __("Bahrain", WDCFM()->prefix),
      "Bangladesh" => __("Bangladesh", WDCFM()->prefix),
      "Barbados" => __("Barbados", WDCFM()->prefix),
      "Belarus" => __("Belarus", WDCFM()->prefix),
      "Belgium" => __("Belgium", WDCFM()->prefix),
      "Belize" => __("Belize", WDCFM()->prefix),
      "Benin" => __("Benin", WDCFM()->prefix),
      "Bhutan" => __("Bhutan", WDCFM()->prefix),
      "Bolivia" => __("Bolivia", WDCFM()->prefix),
      "Bosnia and Herzegovina" => __("Bosnia and Herzegovina", WDCFM()->prefix),
      "Botswana" => __("Botswana", WDCFM()->prefix),
      "Brazil" => __("Brazil", WDCFM()->prefix),
      "Brunei" => __("Brunei", WDCFM()->prefix),
      "Bulgaria" => __("Bulgaria", WDCFM()->prefix),
      "Burkina" => __("Burkina Faso", WDCFM()->prefix),
      "Burundi" => __("Burundi", WDCFM()->prefix),
      "Cambodia" => __("Cambodia", WDCFM()->prefix),
      "Cameroon" => __("Cameroon", WDCFM()->prefix),
      "Canada" => __("Canada", WDCFM()->prefix),
      "Cape Verde" => __("Cape Verde", WDCFM()->prefix),
      "Central African Republic" => __("Central African Republic", WDCFM()->prefix),
      "Chad" => __("Chad", WDCFM()->prefix),
      "Chile" => __("Chile", WDCFM()->prefix),
      "China" => __("China", WDCFM()->prefix),
      "Colombia" => __("Colombia", WDCFM()->prefix),
      "Comoros" => __("Comoros", WDCFM()->prefix),
      "Congo (Brazzaville)" => __("Congo (Brazzaville)", WDCFM()->prefix),
      "Congo" => __("Congo", WDCFM()->prefix),
      "Costa Rica" => __("Costa Rica", WDCFM()->prefix),
      "Cote d'Ivoire" => __("Cote d'Ivoire", WDCFM()->prefix),
      "Croatia" => __("Croatia", WDCFM()->prefix),
      "Cuba" => __("Cuba", WDCFM()->prefix),
      "Curacao" => __("Curacao", WDCFM()->prefix),
      "Cyprus" => __("Cyprus", WDCFM()->prefix),
      "Czech Republic" => __("Czech Republic", WDCFM()->prefix),
      "Denmark" => __("Denmark", WDCFM()->prefix),
      "Djibouti" => __("Djibouti", WDCFM()->prefix),
      "Dominica" => __("Dominica", WDCFM()->prefix),
      "Dominican Republic" => __("Dominican Republic", WDCFM()->prefix),
      "East Timor (Timor Timur" => __("East Timor (Timor Timur)", WDCFM()->prefix),
      "Ecuador" => __("Ecuador", WDCFM()->prefix),
      "Egypt" => __("Egypt", WDCFM()->prefix),
      "El Salvador" => __("El Salvador", WDCFM()->prefix),
      "Equatorial" => __("Equatorial Guinea", WDCFM()->prefix),
      "Eritrea" => __("Eritrea", WDCFM()->prefix),
      "Estonia" => __("Estonia", WDCFM()->prefix),
      "Ethiopia" => __("Ethiopia", WDCFM()->prefix),
      "Fiji" => __("Fiji", WDCFM()->prefix),
      "Finland" => __("Finland", WDCFM()->prefix),
      "France" => __("France", WDCFM()->prefix),
      "Gabon" => __("Gabon", WDCFM()->prefix),
      "Gambia, The" => __("Gambia, The", WDCFM()->prefix),
      "Georgia" => __("Georgia", WDCFM()->prefix),
      "Germany" => __("Germany", WDCFM()->prefix),
      "Ghana" => __("Ghana", WDCFM()->prefix),
      "Greece" => __("Greece", WDCFM()->prefix),
      "Grenada" => __("Grenada", WDCFM()->prefix),
      "Guatemala" => __("Guatemala", WDCFM()->prefix),
      "Guinea" => __("Guinea", WDCFM()->prefix),
      "Guinea-Bissau" => __("Guinea-Bissau", WDCFM()->prefix),
      "Guyana" => __("Guyana", WDCFM()->prefix),
      "Haiti" => __("Haiti", WDCFM()->prefix),
      "Honduras" => __("Honduras", WDCFM()->prefix),
      "Hong Kong" => __("Hong Kong", WDCFM()->prefix),
      "Hungary" => __("Hungary", WDCFM()->prefix),
      "Iceland" => __("Iceland", WDCFM()->prefix),
      "India" => __("India", WDCFM()->prefix),
      "Indonesia" => __("Indonesia", WDCFM()->prefix),
      "Iran" => __("Iran", WDCFM()->prefix),
      "Iraq" => __("Iraq", WDCFM()->prefix),
      "Ireland" => __("Ireland", WDCFM()->prefix),
      "Israel" => __("Israel", WDCFM()->prefix),
      "Italy" => __("Italy", WDCFM()->prefix),
      "Jamaica" => __("Jamaica", WDCFM()->prefix),
      "Japan" => __("Japan", WDCFM()->prefix),
      "Jordan" => __("Jordan", WDCFM()->prefix),
      "Kazakhstan" => __("Kazakhstan", WDCFM()->prefix),
      "Kenya" => __("Kenya", WDCFM()->prefix),
      "Kiribati" => __("Kiribati", WDCFM()->prefix),
      "Korea, North" => __("Korea, North", WDCFM()->prefix),
      "Korea, South" => __("Korea, South", WDCFM()->prefix),
      "Kuwait" => __("Kuwait", WDCFM()->prefix),
      "Kyrgyzstan" => __("Kyrgyzstan", WDCFM()->prefix),
      "Laos" => __("Laos", WDCFM()->prefix),
      "Latvia" => __("Latvia", WDCFM()->prefix),
      "Lebanon" => __("Lebanon", WDCFM()->prefix),
      "Lesotho" => __("Lesotho", WDCFM()->prefix),
      "Liberia" => __("Liberia", WDCFM()->prefix),
      "Libya" => __("Libya", WDCFM()->prefix),
      "Liechtenstein" => __("Liechtenstein", WDCFM()->prefix),
      "Lithuania" => __("Lithuania", WDCFM()->prefix),
      "Luxembourg" => __("Luxembourg", WDCFM()->prefix),
      "Macedonia" => __("Macedonia", WDCFM()->prefix),
      "Madagascar" => __("Madagascar", WDCFM()->prefix),
      "Malawi" => __("Malawi", WDCFM()->prefix),
      "Malaysia" => __("Malaysia", WDCFM()->prefix),
      "Maldives" => __("Maldives", WDCFM()->prefix),
      "Mali" => __("Mali", WDCFM()->prefix),
      "Malta" => __("Malta", WDCFM()->prefix),
      "Marshall Islands" => __("Marshall Islands", WDCFM()->prefix),
      "Mauritania" => __("Mauritania", WDCFM()->prefix),
      "Mauritius" => __("Mauritius", WDCFM()->prefix),
      "Mexico" => __("Mexico", WDCFM()->prefix),
      "Micronesia" => __("Micronesia", WDCFM()->prefix),
      "Moldova" => __("Moldova", WDCFM()->prefix),
      "Monaco" => __("Monaco", WDCFM()->prefix),
      "Mongolia" => __("Mongolia", WDCFM()->prefix),
      "Morocco" => __("Morocco", WDCFM()->prefix),
      "Mozambique" => __("Mozambique", WDCFM()->prefix),
      "Myanmar" => __("Myanmar", WDCFM()->prefix),
      "Namibia" => __("Namibia", WDCFM()->prefix),
      "Nauru" => __("Nauru", WDCFM()->prefix),
      "Nepal" => __("Nepal", WDCFM()->prefix),
      "Netherlands" => __("Netherlands", WDCFM()->prefix),
      "New Zealand" => __("New Zealand", WDCFM()->prefix),
      "Nicaragua" => __("Nicaragua", WDCFM()->prefix),
      "Niger" => __("Niger", WDCFM()->prefix),
      "Nigeria" => __("Nigeria", WDCFM()->prefix),
      "Norway" => __("Norway", WDCFM()->prefix),
      "Oman" => __("Oman", WDCFM()->prefix),
      "Pakistan" => __("Pakistan", WDCFM()->prefix),
      "Palau" => __("Palau", WDCFM()->prefix),
      "Panama" => __("Panama", WDCFM()->prefix),
      "Papua New Guinea" => __("Papua New Guinea", WDCFM()->prefix),
      "Paraguay" => __("Paraguay", WDCFM()->prefix),
      "Peru" => __("Peru", WDCFM()->prefix),
      "Philippines" => __("Philippines", WDCFM()->prefix),
      "Poland" => __("Poland", WDCFM()->prefix),
      "Portugal" => __("Portugal", WDCFM()->prefix),
      "Puerto Rico" => __("Puerto Rico", WDCFM()->prefix),
      "Qatar" => __("Qatar", WDCFM()->prefix),
      "Romania" => __("Romania", WDCFM()->prefix),
      "Russia" => __("Russia", WDCFM()->prefix),
      "Rwanda" => __("Rwanda", WDCFM()->prefix),
      "Saint Kitts and Nevis" => __("Saint Kitts and Nevis", WDCFM()->prefix),
      "Saint Lucia" => __("Saint Lucia", WDCFM()->prefix),
      "Saint Vincent" => __("Saint Vincent", WDCFM()->prefix),
      "Samoa" => __("Samoa", WDCFM()->prefix),
      "San Marino" => __("San Marino", WDCFM()->prefix),
      "Sao Tome and Principe" => __("Sao Tome and Principe", WDCFM()->prefix),
      "Saudi Arabia" => __("Saudi Arabia", WDCFM()->prefix),
      "Senegal" => __("Senegal", WDCFM()->prefix),
      "Serbia and Montenegro" => __("Serbia and Montenegro", WDCFM()->prefix),
      "Seychelles" => __("Seychelles", WDCFM()->prefix),
      "Sierra Leone" => __("Sierra Leone", WDCFM()->prefix),
      "Singapore" => __("Singapore", WDCFM()->prefix),
      "Slovakia" => __("Slovakia", WDCFM()->prefix),
      "Slovenia" => __("Slovenia", WDCFM()->prefix),
      "Solomon Islands" => __("Solomon Islands", WDCFM()->prefix),
      "Somalia" => __("Somalia", WDCFM()->prefix),
      "South Africa" => __("South Africa", WDCFM()->prefix),
      "Spain" => __("Spain", WDCFM()->prefix),
      "Sri Lanka" => __("Sri Lanka", WDCFM()->prefix),
      "Sudan" => __("Sudan", WDCFM()->prefix),
      "Suriname" => __("Suriname", WDCFM()->prefix),
      "Swaziland" => __("Swaziland", WDCFM()->prefix),
      "Sweden" => __("Sweden", WDCFM()->prefix),
      "Switzerland" => __("Switzerland", WDCFM()->prefix),
      "Syria" => __("Syria", WDCFM()->prefix),
      "Taiwan" => __("Taiwan", WDCFM()->prefix),
      "Tajikistan" => __("Tajikistan", WDCFM()->prefix),
      "Tanzania" => __("Tanzania", WDCFM()->prefix),
      "Thailand" => __("Thailand", WDCFM()->prefix),
      "Togo" => __("Togo", WDCFM()->prefix),
      "Tonga" => __("Tonga", WDCFM()->prefix),
      "Trinidad and Tobago" => __("Trinidad and Tobago", WDCFM()->prefix),
      "Tunisia" => __("Tunisia", WDCFM()->prefix),
      "Turkey" => __("Turkey", WDCFM()->prefix),
      "Turkmenistan" => __("Turkmenistan", WDCFM()->prefix),
      "Tuvalu" => __("Tuvalu", WDCFM()->prefix),
      "Uganda" => __("Uganda", WDCFM()->prefix),
      "Ukraine" => __("Ukraine", WDCFM()->prefix),
      "United Arab Emirates" => __("United Arab Emirates", WDCFM()->prefix),
      "United Kingdom" => __("United Kingdom", WDCFM()->prefix),
      "United States" => __("United States", WDCFM()->prefix),
      "Uruguay" => __("Uruguay", WDCFM()->prefix),
      "Uzbekistan" => __("Uzbekistan", WDCFM()->prefix),
      "Vanuatu" => __("Vanuatu", WDCFM()->prefix),
      "Vatican City" => __("Vatican City", WDCFM()->prefix),
      "Venezuela" => __("Venezuela", WDCFM()->prefix),
      "Vietnam" => __("Vietnam", WDCFM()->prefix),
      "Wales" => __("Wales", WDCFM()->prefix),
      "Yemen" => __("Yemen", WDCFM()->prefix),
      "Zambia" => __("Zambia", WDCFM()->prefix),
      "Zimbabwe" => __("Zimbabwe", WDCFM()->prefix),
    );
  ...
  public static function get_states() {
    $states = array(
      "" => "",
      "Alabama" => __("Alabama", WDCFM()->prefix),
      "Alaska" => __("Alaska", WDCFM()->prefix),
      "Arizona" => __("Arizona", WDCFM()->prefix),
      "Arkansas" => __("Arkansas", WDCFM()->prefix),
      "California" => __("California", WDCFM()->prefix),
      "Colorado" => __("Colorado", WDCFM()->prefix),
      "Connecticut" => __("Connecticut", WDCFM()->prefix),
      "Delaware" => __("Delaware", WDCFM()->prefix),
      "District Of Columbia" => __("District Of Columbia", WDCFM()->prefix),
      "Florida" => __("Florida", WDCFM()->prefix),
      "Georgia" => __("Georgia", WDCFM()->prefix),
      "Hawaii" => __("Hawaii", WDCFM()->prefix),
      "Idaho" => __("Idaho", WDCFM()->prefix),
      "Illinois" => __("Illinois", WDCFM()->prefix),
      "Indiana" => __("Indiana", WDCFM()->prefix),
      "Iowa" => __("Iowa", WDCFM()->prefix),
      "Kansas" => __("Kansas", WDCFM()->prefix),
      "Kentucky" => __("Kentucky", WDCFM()->prefix),
      "Louisiana" => __("Louisiana", WDCFM()->prefix),
      "Maine" => __("Maine", WDCFM()->prefix),
      "Maryland" => __("Maryland", WDCFM()->prefix),
      "Massachusetts" => __("Massachusetts", WDCFM()->prefix),
      "Michigan" => __("Michigan", WDCFM()->prefix),
      "Minnesota" => __("Minnesota", WDCFM()->prefix),
      "Mississippi" => __("Mississippi", WDCFM()->prefix),
      "Missouri" => __("Missouri", WDCFM()->prefix),
      "Montana" => __("Montana", WDCFM()->prefix),
      "Nebraska" => __("Nebraska", WDCFM()->prefix),
      "Nevada" => __("Nevada", WDCFM()->prefix),
      "New Hampshire" => __("New Hampshire", WDCFM()->prefix),
      "New Jersey" => __("New Jersey", WDCFM()->prefix),
      "New Mexico" => __("New Mexico", WDCFM()->prefix),
      "New York" => __("New York", WDCFM()->prefix),
      "North Carolina" => __("North Carolina", WDCFM()->prefix),
      "North Dakota" => __("North Dakota", WDCFM()->prefix),
      "Ohio" => __("Ohio", WDCFM()->prefix),
      "Oklahoma" => __("Oklahoma", WDCFM()->prefix),
      "Oregon" => __("Oregon", WDCFM()->prefix),
      "Pennsylvania" => __("Pennsylvania", WDCFM()->prefix),
      "Rhode Island" => __("Rhode Island", WDCFM()->prefix),
      "South Carolina" => __("South Carolina", WDCFM()->prefix),
      "South Dakota" => __("South Dakota", WDCFM()->prefix),
      "Tennessee" => __("Tennessee", WDCFM()->prefix),
      "Texas" => __("Texas", WDCFM()->prefix),
      "Utah" => __("Utah", WDCFM()->prefix),
      "Vermont" => __("Vermont", WDCFM()->prefix),
      "Virginia" => __("Virginia", WDCFM()->prefix),
      "Washington" => __("Washington", WDCFM()->prefix),
      "West Virginia" => __("West Virginia", WDCFM()->prefix),
      "Wisconsin" => __("Wisconsin", WDCFM()->prefix),
      "Wyoming" => __("Wyoming", WDCFM()->prefix),
    );
  ...
  public static function localize_ui_datepicker() {
    return 'jQuery(document).ready(function(jQuery){
      jQuery.datepicker.setDefaults( {
        "closeText":"' . __('Done', WDCFM()->prefix) . '",
        "prevText":"' . __('Prev', WDCFM()->prefix) . '",
        "nextText":"' . __('Next', WDCFM()->prefix) . '",
        "currentText":"' . __('Today', WDCFM()->prefix) . '",
        "monthNames":["' . __('January', WDCFM()->prefix) . '","' . __('February', WDCFM()->prefix) . '","' . __('March', WDCFM()->prefix) . '","' . __('April', WDCFM()->prefix) . '","' . __('May', WDCFM()->prefix) . '","' . __('June', WDCFM()->prefix) . '","' . __('July', WDCFM()->prefix) . '","' . __('August', WDCFM()->prefix) . '","' . __('September', WDCFM()->prefix) . '","' . __('October', WDCFM()->prefix) . '","' . __('November', WDCFM()->prefix) . '","' . __('December', WDCFM()->prefix) . '"],
        "monthNamesShort":["' . __('Jan', WDCFM()->prefix) . '","' . __('Feb', WDCFM()->prefix) . '","' . __('Mar', WDCFM()->prefix) . '","' . __('Apr', WDCFM()->prefix) . '","' . __('May', WDCFM()->prefix) . '","' . __('Jun', WDCFM()->prefix) . '","' . __('Jul', WDCFM()->prefix) . '","' . __('Aug', WDCFM()->prefix) . '","' . __('Sep', WDCFM()->prefix) . '","' . __('Oct', WDCFM()->prefix) . '","' . __('Nov', WDCFM()->prefix) . '","' . __('Dec', WDCFM()->prefix) . '"],
        "dayNames":["' . __('Sunday', WDCFM()->prefix) . '","' . __('Monday', WDCFM()->prefix) . '","' . __('Tuesday', WDCFM()->prefix) . '","' . __('Wednesday', WDCFM()->prefix) . '","' . __('Thursday', WDCFM()->prefix) . '","' . __('Friday', WDCFM()->prefix) . '","' . __('Saturday', WDCFM()->prefix) . '"],
        "dayNamesShort":["' . __('Sun', WDCFM()->prefix) . '","' . __('Mon', WDCFM()->prefix) . '","' . __('Tue', WDCFM()->prefix) . '","' . __('Wed', WDCFM()->prefix) . '","' . __('Thu', WDCFM()->prefix) . '","' . __('Fri', WDCFM()->prefix) . '","' . __('Sat', WDCFM()->prefix) . '"],
       "dayNamesMin":["' . __('Su', WDCFM()->prefix) . '","' . __('Mo', WDCFM()->prefix) . '","' . __('Tu', WDCFM()->prefix) . '","' . __('We', WDCFM()->prefix) . '","' . __('Th', WDCFM()->prefix) . '","' . __('Fr', WDCFM()->prefix) . '","' . __('Sa', WDCFM()->prefix) . '"]
      });
    })';
  }
