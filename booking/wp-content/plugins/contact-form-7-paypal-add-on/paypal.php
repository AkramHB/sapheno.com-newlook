	...
	if ( is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) ) {
		...
		function cf7pp_before_send_mail( $cf7 ) {
			...
			if ($submission_orig) {
				...
				if ($enable == "1" && $options['redirect'] == "1") {
					$site_url = get_site_url();
					$path = $site_url.'/?cf7pp_redirect='.$postid;
		...
		// hook into contact form 7 - after send - for redirect method 1
		...
		// hook into contact form 7 - after send - redirect method 2
		...
		function cf7pp_editor_panels ( $panels ) {
			...
			$new_page = array(
				'PayPal' => array(
					'title' => __( 'PayPal', 'contact-form-7-paypal' ),
		...
		wp_die( __( "You do not have sufficient permissions to access this page." ) );
			...
			When go to your list of contact forms, make a new form or edit an existing form, you will see a new tab called 'PayPal'. Here you can 
			setup individual settings for that specific contact form.
			
			<br /><br />
			
			On this page, you can setup your general PayPal settings which will be used for all of your contact forms.
			...
			<b>Note:</b> If you experience problems with the form not redirecting to PayPal, try changing the redirect method setting on the bottom of this page.
			...
			<b>Live Account: </b><input type='text' name='liveaccount' value='<?php echo $options['liveaccount']; ?>'> Required
			...
			<b>Sandbox Account: </b><input type='text' name='sandboxaccount' value='<?php echo $options['sandboxaccount']; ?>'> Optional
			...
			<b>Cancel URL: </b>
			<input type='text' name='cancel' value='<?php echo $options['cancel']; ?>'> Optional <br />
			...
			<b>Return URL: </b>
			<input type='text' name='return' value='<?php echo $options['return']; ?>'> Optional <br />
			...
			<div class="dashicons dashicons-yes" style="margin-bottom: 6px;"></div> Separate PayPal account per form<br />
		...
		function cf7pp_my_admin_notice() {
			?>
			<div class="error">
				<p><?php _e( '<b>Contact Form 7 PayPal Add-on:</b> Contact Form 7 is not installed and / or active! ', 'cf7pp' ); ?></p>
			</div>
			<?php
		}
