...
//                }
		...
		$DOT->models->availability->set($reservation->calendar_id);
             ...
             * @param reservation_id (integer): reservation ID
             */
            function setAirbnb($calendar_id,
                               $check_in,
                               $check_out){
		global $DOT;
                global $wpdb;
                ...
                if($source != 'pinpoint') {
                    ...
                    $days_history = $DOPBSP->classes->backend_calendar_schedule->daysHistory($check_in, 
                                                                                             $check_out,
                                                                                             $calendar_id);
                }
                ...
                if(!empty($days_history)) {
                    ...
                    $check_in_d = strtotime( $check_in.' 12:00' );
                    $check_out_d = strtotime( $check_out.' 12:00' );
                    
                    if ($settings_calendar->days_morning_check_out == 'true'){
//                        $check_out_new = date('Y-m-d', strtotime($check_out));
//                        // check_out - 1 day
...
//                        $check_out_d = strtotime( $check_out.' 12:00' );
                    ...
                    if($check_out != '') {
                        ...
                        for ( $i = $check_in_d; $i <= $check_out_d; $i = $i + 86400 ) {
                            $day = date( 'Y-m-d', $i );
                            $year = date( 'Y', $i );
                            $price_min  = 1000000000;
                            $price_max  = 0;
		    ...
		    $DOT->models->availability->set($calendar_id);
                 ...
                 * Get info data.
                 */        
                $info_info = $this->getFormInfo(array());
                $info_body = $this->getFormInfo($this->getFormInfo(array()), 'body');
                ...
                if ($check_out == ''){
                    $day = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$DOPBSP->tables->days.' WHERE calendar_id=%d AND day="%s"',
                                                         $calendar_id, $check_in));
                }
                else{
                    if ($settings_calendar->days_morning_check_out == 'true'){
                        $days = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$DOPBSP->tables->days.' WHERE calendar_id=%d AND day>="%s" AND day<"%s"',
                                                                  $calendar_id, $check_in, $check_out));
                    }
                    else{
                        $days = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$DOPBSP->tables->days.' WHERE calendar_id=%d AND day>="%s" AND day<="%s"',
                                                                  $calendar_id, $check_in, $check_out));
                        ...
                        foreach ($day_data->hours as $key => $hour){
                            if ($start_hour <= $key 
                                ...
                                if($calendar->default_availability != '') {
                                    ...
                                    if($default_availability->hours->$key != 'available'
                                      && $default_availability->hours->$key != 'special') {
                                        return false;
                                    }
                                } else {
                                    return false;
