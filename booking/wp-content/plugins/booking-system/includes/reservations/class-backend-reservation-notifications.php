    ...
    if (!class_exists('DOPBSPBackEndReservationNotifications')){
        class DOPBSPBackEndReservationNotifications extends DOPBSPBackEndReservation{
            ...
            function send($reservation_id,
                ...
                if($settings_notifications->clickatell_send_book_admin == 'true'
                   ...
                   || $settings_notifications->clickatell_send_rejected == 'true') {
                    ...
                    $sms_message = $this->getSMSMessage($DOPBSP->text('SETTINGS_NOTIFICATIONS_SMS_CLICKATELL_'.strtoupper($template)),
                                                        $reservation,
                                                        $calendar,
                                                        $settings_calendar,
                                                        $settings_payment); 
                    ...
                    if (strpos($template, 'admin') !== false){
                        ...
                        if($settings_notifications->{'clickatell_send_'.$template} == 'true'
                            ...
                            foreach($admin_phones as $admin_phone) {
                                ...
                                for ($i=($admin_phone->phone[0] == '0' ? 1:0); $i<strlen($admin_phone->phone); $i++){
                                    $phone .= $admin_phone->phone[$i];
                                }
                                
                                $DOPBSP->classes->clickatell->send($phone,
                                                                   $sms_message);
                            }
                        }
                    } else {
                        ...
                        if($settings_notifications->{'clickatell_send_'.$template} == 'true') {
                            $DOPBSP->classes->clickatell->send($reservation->phone,
                                                               $sms_message);
                        }
                    }                   
            ...
            function getSMSMessage($message,
                ...
                $message = $this->getShortcodes($message,
                                                $reservation);  
                $message = str_replace('|DETAILS|', $this->getSMSDetails($reservation, $calendar, $settings_calendar), $message);
                ...
                foreach($reservation as $key => $value) {
                    ...
                    if($key == 'form') {
                        ...
                        for ($i=0; $i<count($form); $i++){
                            ...
                            $message = str_replace('|'.strtoupper(str_replace(' ','_',$form_item['translation'])).'|', $form_item['value'], $message);
                        }
                    } else if($key != 'days_hours_history'
                             && $key != 'deposit'
                             && $key != 'coupon'
                             && $key != 'extras'
                             && $key != 'fees'
            ...
            function getDetails($reservation,
                ...
                if($reservation->ip != '' && DOPBSP_CONFIG_VIEW_IP_ADDRESS) {
                    array_push($info, $this->getInfo($DOPBSP->text('RESERVATIONS_RESERVATION_IP_ADDRESS'),
                                       $reservation->ip));
                }
                ...
                switch ($reservation->payment_method){
                    ...
                    case 'woocommerce':
                         ...
                         * Order ID
                         */
                        array_push($info, $this->getInfo($DOPBSP->text('ORDER_PAYMENT_METHOD_WOOCOMMERCE_ORDER_ID'),
            ...
            function getSMSDetails($reservation,
                 ...
                 * Check in data.
                 */
                array_push($info, $DOPBSP->text('SEARCH_FRONT_END_CHECK_IN').' - '.$DOPBSP->classes->prototypes->setDateToFormat($reservation->check_in, 
                                                                                               $settings_calendar->date_type, 
                                                                                               array($DOPBSP->text('MONTH_JANUARY'), 
                                                                                                     $DOPBSP->text('MONTH_FEBRUARY'),  
                                                                                                     $DOPBSP->text('MONTH_MARCH'),  
                 ...
                 */
                if ($reservation->check_out != ''){
                    array_push($info, $DOPBSP->text('SEARCH_FRONT_END_CHECK_OUT').' - '.$DOPBSP->classes->prototypes->setDateToFormat($reservation->check_out, 
                                                                                                   $settings_calendar->date_type, 
                                                                                                   array($DOPBSP->text('MONTH_JANUARY'), 
                                                                                                         $DOPBSP->text('MONTH_FEBRUARY'),  
                                                                                                         $DOPBSP->text('MONTH_MARCH'),  
                 ...
                 */
                if ($reservation->start_hour != ''){
                    array_push($info, $DOPBSP->text('SEARCH_FRONT_END_START_HOUR').' - '.$settings_calendar->hours_ampm == 'true' ? $DOPBSP->classes->prototypes->getAMPM($reservation->start_hour):$reservation->start_hour);
                }
                 ...
                 */
                if ($reservation->end_hour != ''){
                    array_push($info, $DOPBSP->text('SEARCH_FRONT_END_END_HOUR').' - '.$settings_calendar->hours_ampm == 'true' ? $DOPBSP->classes->prototypes->getAMPM($reservation->end_hour):$reservation->end_hour);
                }
                 ...
                 */
                if ($settings_calendar->sidebar_no_items_enabled == 'true'){
                    array_push($info, $DOPBSP->text('SEARCH_FRONT_END_NO_ITEMS').' - '.$reservation->no_items);
                }
                 ...
                 */
                if ($reservation->price > 0){
                    array_push($info, $DOPBSP->text('RESERVATIONS_RESERVATION_FRONT_END_PRICE').' - '.$DOPBSP->classes->price->set($reservation->price,
                                                                                  $reservation->currency,
                                                                                  $settings_calendar->currency_position),
                                                     'price');
                }
                ...
                switch ($reservation->payment_method){
                    case 'none':
                        array_push($info, $DOPBSP->text('ORDER_PAYMENT_METHOD').' - '.$DOPBSP->text('ORDER_PAYMENT_METHOD_NONE'));
                        break;
                    case 'default':
                        array_push($info, $DOPBSP->text('ORDER_PAYMENT_METHOD').' - '.$DOPBSP->text('ORDER_PAYMENT_METHOD_ARRIVAL'));
                        break;
                    default:
                        array_push($info, $DOPBSP->text('ORDER_PAYMENT_METHOD').' - '.$DOPBSP->text('SETTINGS_PAYMENT_GATEWAYS_'.strtoupper($reservation->payment_method)));
                        
                        /*
                         * Transaction ID
                         */
                        array_push($info, $DOPBSP->text('ORDER_PAYMENT_METHOD_TRANSACTION_ID').' - '.$reservation->transaction_id);
                }
                 ...
                 */
                if ($reservation->price_total > 0){
                    array_push($info, $DOPBSP->text('RESERVATIONS_RESERVATION_FRONT_END_TOTAL_PRICE').' - '.$DOPBSP->classes->price->set($reservation->price_total,
                                                                                  $reservation->currency,
                                                                                  $settings_calendar->currency_position),
                                                     'price-total');
                }
                ...
                if ($reservation->deposit_price > 0){
                    ...
                    array_push($info, $DOPBSP->text('RESERVATIONS_RESERVATION_FRONT_END_DEPOSIT').' - '.($deposit->price_type == 'percent' ? '&#9632;&nbsp;'.$deposit->price.'%, ':'').
                                                     $DOPBSP->classes->price->set($reservation->deposit_price,
                                                                                  $reservation->currency,
                                                                                  $settings_calendar->currency_position),
                                                     'price');
                    array_push($info, $DOPBSP->text('RESERVATIONS_RESERVATION_FRONT_END_DEPOSIT_LEFT').' - '.$DOPBSP->classes->price->set($reservation->price_total-$reservation->deposit_price,
                                                                                  $reservation->currency,
                                                                                  $settings_calendar->currency_position),
                                                     'price-total');
                }
                
                return $DOPBSP->text('RESERVATIONS_RESERVATION_DETAILS_TITLE').': '.implode(', ', $info);
            }
